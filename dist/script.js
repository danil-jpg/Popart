...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _modules_services_requests__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ../modules/services/requests */ "./src/js/modules/services/requests.js");
...
const calculator = (size, material, options, promocode, result, form) => {
  ...
  const calcFunction = () => {
    ...
    Object(_modules_services_requests__WEBPACK_IMPORTED_MODULE_0__["postData"])("../../assets/server.php", formData).then(res => console.log(res));
  };
...
/* harmony import */ var _modules_services_requests__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ../modules/services/requests */ "./src/js/modules/services/requests.js");
...
const forms = () => {
  ...
  upload.forEach(item => {
    item.addEventListener("input", () => {
      ...
      item.previousElementSibling.textContent = name; // previousElementSibling содержит предыдущий элемент, находящийся в этом же родителе.
  ...
  form.forEach(item => {
    item.addEventListener("submit", e => {
      ...
      let formData = new FormData(item),
          api;
      item.closest(".popup-design") || item.classList.contains("calc_form") ? api = path.designer : api = path.question; // если попар тру,мы присваеваем апи путь,если нет то questijon
      ...
      Object(_modules_services_requests__WEBPACK_IMPORTED_MODULE_1__["postData"])(api, formData).then(res => {
        console.log(res);
...
const modals = () => {
  ...
  function openByScroll(selector) {
    window.addEventListener("scroll", () => {
      if (!btnPressed && window.pageYOffset + document.documentElement.clientHeight >= document.documentElement.scrollHeight) {
        document.querySelector(selector).click(); // вот этот клик - это программное использование событие,то есть,если условие выполниться
  ...
  bindModal(".button-consultation", ".popup-consultation", ".popup-content .popup-close");
...
const showMoreStyles = (trigger, wrapper) => {
  ...
  btn.addEventListener("click", function () {
    Object(_modules_services_requests__WEBPACK_IMPORTED_MODULE_0__["getResource"])("assets/db.json") // скомпилиный скрипт заходит в папку и берет датабес
    .then(res => createCards(res.styles)).catch(eror => console.log(eror));
    this.remove(); // res это результат преведущей операции
  });
...
const sliders = (slides, dir, prev, next, time) => {
  ...
  function showSlides(n) {
    ...
    items[slideIndex - 1].style.display = "block"; // slideIndex начинаеться с 1,но слайды идут с 0 ,следовательно 0 слайд = 1
  }
